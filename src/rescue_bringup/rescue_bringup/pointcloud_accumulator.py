...
class PointCloudAccumulator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('prefer_pcl_tools', True)
    ...
    def _publish_accumulated(self):
        ...
        if rgb is not None and len(rgb) == len(xyz):
            ...
            data = np.column_stack([xyz.astype(np.float32), packed]).astype(np.float32)
            msg.fields = [
    ...
    def _export_ply(self) -> str:
        xyz, rgb, path, ts = self._prepare_export_cloud()

        if bool(self.get_parameter('prefer_pcl_tools').value):
            pcl_path = self._export_ply_with_pcl(xyz, rgb, path, ts)
            if pcl_path is not None:
                return pcl_path

        self._write_ply_ascii(path, xyz, rgb, ts)
        return path

    def _prepare_export_cloud(self) -> Tuple[np.ndarray, Optional[np.ndarray], str, str]:
        v     = self.get_parameter('voxel_size').value
        rgb_src = self._rgb.copy() if (
            self._rgb is not None and len(self._rgb) == len(self._xyz)
        ) else None
        xyz, rgb = _voxel_filter(self._xyz.copy(), rgb_src, v)
        ...
        return xyz, rgb, path, ts

    def _export_ply_with_pcl(self, xyz: np.ndarray, rgb: Optional[np.ndarray],
                             path: str, ts: str) -> Optional[str]:
        tool = shutil.which('pcl_pcd2ply')
        if tool is None:
            self.get_logger().warn(
                'PCL no disponible (pcl_pcd2ply no encontrado); '
                'se usa exportador PLY Python.')
            return None
        ...
        try:
            with tempfile.NamedTemporaryFile(
                    mode='w', suffix='.pcd', prefix='rescue_cloud_',
                    dir=out, delete=False) as tmp:
                tmp_path = tmp.name
                self._write_pcd_ascii(tmp, xyz, rgb)

            proc = subprocess.run(
                [tool, '-format', '0', tmp_path, path],
            ...
            if proc.returncode == 0 and os.path.exists(path) and os.path.getsize(path) > 0:
                self._inject_ply_comments(path, ts)
                self.get_logger().info(f'PLY exportado con PCL → {path}')
                return path

            err = (proc.stderr or proc.stdout or '').strip()
            self.get_logger().warn(
                f'pcl_pcd2ply falló ({proc.returncode}): {err[:180]}; '
                'se usa exportador PLY Python.')
        except Exception as exc:
            self.get_logger().warn(
                f'No se pudo exportar con PCL: {exc}; se usa exportador PLY Python.')
        finally:
            if tmp_path:
                try:
                    os.remove(tmp_path)
    ...
    def _write_pcd_ascii(self, f, xyz: np.ndarray, rgb: Optional[np.ndarray]) -> None:
        n = len(xyz)
        has_rgb = rgb is not None and len(rgb) == n
        if has_rgb:
            f.write(
                '# .PCD v0.7 - Point Cloud Data file generated by Pedro Rescue\n'
                'VERSION 0.7\n'
                'FIELDS x y z rgb\n'
                'SIZE 4 4 4 4\n'
                'TYPE F F F F\n'
            ...
            for i in range(n):
                x, y, z = xyz[i]
                f.write(f'{x:.6f} {y:.6f} {z:.6f} {packed[i]:.8e}\n')
            return
    ...
    def _ply_comments(self, ts: str) -> bytes:
        team = self.get_parameter('team_name').value
        miss = self.get_parameter('mission').value
        lines = [
            f'comment Team: {team}\n',
            f'comment Mission: {miss}\n',
            f'comment StartTime: {ts.replace("-", ":")}\n',
            'comment Origin: center-front-robot floor height start position\n',
            'comment Axes: +Y=forward +Z=up scale=meters\n',
        ]
        return ''.join(lines).encode('ascii')

    def _inject_ply_comments(self, path: str, ts: str) -> None:
        try:
            with open(path, 'rb') as f:
                data = f.read()
            if b'comment Team:' in data[:512]:
                return
            first = data.find(b'\n')
            ...
            with open(path, 'wb') as f:
                f.write(data[:second + 1])
                f.write(self._ply_comments(ts))
                f.write(data[second + 1:])
        except Exception as exc:
            self.get_logger().warn(f'No se pudieron insertar comentarios PLY: {exc}')

    def _write_ply_ascii(self, path: str, xyz: np.ndarray,
                         rgb: Optional[np.ndarray], ts: str) -> None:
        n = len(xyz)
        with open(path, 'w') as f:
            f.write('ply\nformat ascii 1.0\n')
            f.write(self._ply_comments(ts).decode('ascii'))
